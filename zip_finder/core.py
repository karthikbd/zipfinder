...
class ZipFinderDatabase:
    ...
    def _load_from_pickles(self) -> None:
        ...
        Streams gzip → pickle.load to avoid holding a full decompressed
        copy in memory alongside the parsed objects.
        """
        cdata = pkgutil.get_data(__name__, "data/country_index.pkl.gz")
        pdata = pkgutil.get_data(__name__, "data/postal_index.pkl.gz")
        ZipFinderDatabase._country_index = pickle.load(
            gzip.GzipFile(fileobj=io.BytesIO(cdata))
        )
        ZipFinderDatabase._postal_index = pickle.load(
            gzip.GzipFile(fileobj=io.BytesIO(pdata))
        )
